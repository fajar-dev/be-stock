...
export class StockVariantController {
    ...
    async index(c: Context) {
        ...
        return ApiResponse.paginate(c, StockVariantSerializer.collection(variants), total, page, limit, 'Stock variants retrieved successfully');
    }
    ...
    async show(c: Context) {
        ...
        return ApiResponse.success(c, StockVariantSerializer.single(variant), 'Stock variant retrieved successfully', 200);
    }

    async store(c: Context) {
        const data = c.req.valid('json' as never) as CreateStockVariantValidator;
        const variants = await this.service.createBulk(data);
        return ApiResponse.success(c, StockVariantSerializer.collection(variants), 'Stock variants created successfully', 201);
    }
}
